"""diagnostic/p2_probe/_bootstrap_delta.py — 只读：图像级配对 bootstrap，给 P2 vs Baseline 的 Δ 置信区间。

动机：官方口径的 Δsmall = +0.00236（base 0.05625）是否只是噪声？
      预注册判定规则（§12.8）本身不要求这一步，但它直接回答 P2 探针的科学命题
      「P2 是否真的改善小目标」，因此额外做一次。
...
DIRS = {
    ...
    "P2_best": ROOT / "diagnostic/p2_probe/p2_scratch_best/results",
}
...
def main():
    ...
    for it in range(args.iters):
        ...
        for k in keys:
            deltas[k].append(m["P2_best"][k] - m["Baseline"][k])
        if (it + 1) % 25 == 0:
    ...
    for k in keys:
        d = np.array(deltas[k])
        pt = point["P2_best"][k] - point["Baseline"][k]
