...
职责：加载训练好的 best.pt（默认 E7 YOLO11m RGBD 中期融合模型，imgsz=1024）→ 在 test 集上推理
...
用法:
  python scripts/predict.py                                    # 默认 E7 best.pt
...
# E7 默认权重 / 训练配置 / 测试输入（YOLO11m RGBD，imgsz=1024，best mAP50-95=0.50878）
DEFAULT_WEIGHTS = "runs/urban_multimodal_det_e7_yolo11m_rgbd_1024/weights/best.pt"
DEFAULT_TRAIN_CONFIG = "configs/train_e7.yaml"
...
def _parse_args():
    ...
    parser.add_argument("--imgsz", type=int, default=1024, help="推理输入尺寸，默认 1024（与 E7 训练一致）")
...
def main():
    ...
    for paths, imgs, _info in loader:
        ...
        with torch.no_grad():
            preds = nn_model(x)
        out = non_max_suppression(preds, args.conf, args.iou, nc=nc, max_det=args.max_det)
        ...
        if args.tta:
            ...
            with torch.no_grad():
                preds_f = nn_model(xf)
            out_f = non_max_suppression(preds_f, args.conf, args.iou, nc=nc, max_det=args.max_det)
            W_lb = x.shape[3]
            for i in range(len(out)):
                df = out_f[i]
