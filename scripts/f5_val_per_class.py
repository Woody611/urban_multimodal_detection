"""scripts/f5_val_per_class.py — F5 best.pt 逐类指标 dump（与 F4 逐类 CSV 对齐口径）。

复刻训练内置 val：conf=0.001 / iou=0.7 / max_det=300 / imgsz=1280 / rect=False。
输出 reports/f5_class_metrics.csv（逐类 AP50/AP75/AP50-95/P/R）。
"""
...
WEIGHTS = "runs/urban_multimodal_det_yolo11_rgbd_f5_l1280/weights/best.pt"
DATA = "data/processed/depth_split_train/dataset.yaml"
OUT = ROOT / "reports" / "f5_class_metrics.csv"
...
metrics = model.val(
    ...
    project=str(ROOT / "runs" / "audit"), name="f5_val", exist_ok=True,
)
